...
description: Use whenever building or redesigning a frontend interface — websites, landing pages, dashboards, product UI, components — and the result should feel authored by a strong human designer with a distinctive point of view, not averaged from template patterns. Trigger on requests to design, build, style, or polish any UI, even if the user doesn't explicitly ask for "distinctive" or "creative" design. Enforces a committed art direction, a functional signature element, a token-driven design system, complete interaction/failure states, accessibility, and a strict checklist against generic AI-template aesthetics (Inter + purple gradient + rounded cards + generic icons).
---
